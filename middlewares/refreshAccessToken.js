...
export const verifyAccessToken = async (req, res, next) => {
    ...
    try
    {
        ...
        const userData = await userModel.findOne({ accountId: user.accountId, userLocationId: user.userLocationId, companyId: user.companyId });
        if (!userData) {
        ...
        if (userData.expiryDate < Date.now()) {
            ...
            await userModel.updateMany({ companyId: user.companyId, userLocationId: user.userLocationId }, { accessToken: data.access_token, refreshToken: data.refresh_token, expiryDate: newexpiryDate, scope: data.scope });
